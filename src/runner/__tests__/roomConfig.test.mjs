...
describe("slopcodeRoomConfig", () => {
  it("undefined -> full bench (no problem_set)", () => {
    expect(slopcodeRoomConfig(undefined)).toEqual({ game_type: "slopcode" });
  });

  it("null -> full bench (no problem_set)", () => {
    expect(slopcodeRoomConfig(null)).toEqual({ game_type: "slopcode" });
  });

  it('empty string "" -> full bench (no problem_set)', () => {
    expect(slopcodeRoomConfig("")).toEqual({ game_type: "slopcode" });
  });

  it('"all" (any case) -> full bench (no problem_set)', () => {
    expect(slopcodeRoomConfig("all")).toEqual({ game_type: "slopcode" });
    expect(slopcodeRoomConfig("ALL")).toEqual({ game_type: "slopcode" });
    expect(slopcodeRoomConfig("All")).toEqual({ game_type: "slopcode" });
    expect(slopcodeRoomConfig("  all  ")).toEqual({ game_type: "slopcode" });
  });
  ...
  for (const key of presets) {
    it(`preset "${key}" (mixed case) -> problem_set canonical lowercase`, () => {
      expect(slopcodeRoomConfig(key)).toEqual({ game_type: "slopcode", problem_set: key });
      // mixed case variant
      const mixed = key
        .split("")
        .map((c, i) => (i % 2 === 0 ? c.toUpperCase() : c))
        .join("");
      expect(slopcodeRoomConfig(mixed)).toEqual({ game_type: "slopcode", problem_set: key });
    });
  }

  it("a non-preset single string is treated as a single problem name", () => {
    expect(slopcodeRoomConfig("xjq")).toEqual({ game_type: "slopcode", problems: ["xjq"] });
  });
